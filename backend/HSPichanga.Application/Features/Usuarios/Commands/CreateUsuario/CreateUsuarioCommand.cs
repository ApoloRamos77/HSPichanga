...
public record CreateUsuarioResult(Guid Id, string Email, string TempPassword);
...
public class CreateUsuarioCommandHandler : IRequestHandler<CreateUsuarioCommand, CreateUsuarioResult>
{
    ...
    public CreateUsuarioCommandHandler(IUnitOfWork uow, IEmailService email)
    {
        _uow = uow;
        _email = email;
    }
    ...
    public async Task<CreateUsuarioResult> Handle(CreateUsuarioCommand request, CancellationToken cancellationToken)
    {
        ...
        if (await _uow.Usuarios.ExisteEmailAsync(emailNorm, cancellationToken))
        ...
        var usuario = Usuario.Crear(request.NombreCompleto, emailNorm, hash, request.Rol, request.Telefono);
        ...
        return new CreateUsuarioResult(usuario.Id, emailNorm, tempPass);
    }
}
